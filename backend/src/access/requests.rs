...
use crate::users::models::{User, UserList, UserRequest, SearchUser, UserResponse};
...
pub fn get_user(id_token: &str, database_connection: &MysqlConnection) -> Option<User> {
    let mut client = google_signin::Client::new();
    client.audiences.push(String::from("743489940041-kd5b4r3l4tiohluea0omifnhdf7db78t.apps.googleusercontent.com")); // required
        ...
        Ok(info) => {
            match info.email {
                Some(email) => {
                    ...
                    match handle_user(search_request, Some(User{id:0,first_name:String::from("root"),last_name:String::from("root"),banner_id:0,email:None}), database_connection) {
                        Ok(found_user) => {
                            match found_user {
                                UserResponse::OneUser(user) => Some(user),
                                UserResponse::ManyUsers(user_list) => {let user = user_list.users[0]; Some(user)},
...
pub fn check_to_run(
    requested_user_id: Option<User>,
    access_type: &str,
    database_connection: &MysqlConnection
) -> Result<(), WebdevError> {
    match requested_user_id {
        Some(user) => {
            match check_user_access(user.id, String::from(access_type), database_connection) {
                Ok(access) => {
                    if access {
                        Ok(())
                    } else {
...
pub fn handle_access(
    ...
    match request {
        AccessRequest::GetAccess(id) => {
            match check_to_run(requested_user, "GetAccess", database_connection) {
                Ok(()) => get_access(id, database_connection).map(|a| AccessResponse::OneAccess(a)),
                Err(e) => Err(e),
            }
        },
        AccessRequest::CreateAccess(access) => {
            match check_to_run(requested_user, "CreateAccess", database_connection) {
                Ok(()) => create_access(access, database_connection).map(|a| AccessResponse::OneAccess(a)),
                Err(e) => Err(e),
            }
        },
        AccessRequest::UpdateAccess(id, access) => {
            match check_to_run(requested_user, "UpdateAccess", database_connection) {
                Ok(()) => update_access(id, access, database_connection).map(|_| AccessResponse::NoResponse),
                Err(e) => Err(e),
            }
        },
        AccessRequest::DeleteAccess(id) => {
            match check_to_run(requested_user, "DeleteAccess", database_connection) {
                Ok(()) => delete_access(id, database_connection).map(|_| AccessResponse::NoResponse),
                Err(e) => Err(e),
            }
        }
    }
...
pub fn handle_user_access(
    ...
    match request {
        UserAccessRequest::SearchAccess(user_access) => {
            match check_to_run(requested_user, "GetUserAccess", database_connection) {
                Ok(()) => search_user_access(user_access, database_connection).map(|u| UserAccessResponse::ManyUserAccess(u)),
                Err(e) => Err(e),
            }
        },
        UserAccessRequest::GetAccess(permission_id) => {
            match check_to_run(requested_user, "GetUserAccess", database_connection) {
                Ok(()) => get_user_access(permission_id, database_connection).map(|a| UserAccessResponse::OneUserAccess(a)),
                Err(e) => Err(e),
            }
        },
        UserAccessRequest::CheckAccess(user_id, access_name) => {
            check_user_access(user_id, access_name, database_connection).map(|s| UserAccessResponse::AccessState(s))
        },
        UserAccessRequest::CreateAccess(user_access) => {
            match check_to_run(requested_user, "CreateUserAccess", database_connection) {
                Ok(()) => create_user_access(user_access, database_connection).map(|a| UserAccessResponse::OneUserAccess(a)),
                Err(e) => Err(e),
            }
        },
        UserAccessRequest::UpdateAccess(id, user_access) => {
            match check_to_run(requested_user, "UpdateUserAccess", database_connection) {
                Ok(()) => update_user_access(id, user_access, database_connection).map(|_| UserAccessResponse::NoResponse),
                Err(e) => Err(e),
            }
        },
        UserAccessRequest::DeleteAccess(id) => {
            match check_to_run(requested_user, "DeleteUserAccess", database_connection) {
                Ok(()) => delete_user_access(id, database_connection).map(|_| UserAccessResponse::NoResponse),
                Err(e) => Err(e),
            }
        }
    }
...
fn check_user_access(
    ...
    if access_name != "RootAccess" {
        match check_user_access(user_id, String::from("RootAccess"), database_connection) {
            Ok(access) => if access { return Ok(true); },
            Err(_e) => {},
        }
    }
    ...
    let found_user_accesses = user_access_schema::table
        .inner_join(access_schema::table)
        .select((user_access_schema::user_id,
